...
- File-backed agent state in `.data/agentid-db.json`, seeded from the MVP records.
- Product status endpoint: `GET /api/agentid/status`.
- Agents endpoints for listing, creating, quoting, and paid execution.
...
- `app/api/agentid/agents` exposes creation and listing.
- `app/api/agentid/agents/[slug]/quote` returns a payment requirement.
- `app/api/agentid/agents/[slug]/run` blocks unpaid access with `402 Payment Required`, records paid runs, and emits `payment-response`.
- `app/api/mcp/agentid` maps agent tools to discovery, quotes, prepared runs, and stats.
